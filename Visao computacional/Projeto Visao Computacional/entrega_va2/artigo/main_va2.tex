...
\begin{tabular}{lcc}
\toprule
\textbf{Métrica} & \textbf{Input (original)} & \textbf{Output (inpainted)} \\
\midrule
LPIPS temporal      & 0.0234 & \textsuperscript{$\dagger$} \\
Flow cons.\ L1      & 0.0055 & \textsuperscript{$\dagger$} \\
PSNR consec.\ (dB)  & 28.61  & \textsuperscript{$\dagger$} \\
SSIM consec.        & 0.9277 & \textsuperscript{$\dagger$} \\
\bottomrule
\multicolumn{3}{l}{\small $^\dagger$ Valores obtidos na execução da Seção~7 do notebook.}
\end{tabular}
\end{table}
...
\caption{Comparação de frames: linha superior = vídeo original; linha inferior = vídeo inpaintado (retângulo vermelho indica região removida). O preenchimento por TELEA é visualmente coerente mas introduz inconsistências temporais detectáveis pelas métricas.}
\label{fig:inpainted}
\end{figure}

A hipótese de coerência das métricas é: o vídeo inpaintado deve apresentar LPIPS temporal e Flow Consistency L1 \emph{maiores} que o original (a região preenchida introduz variação local não presente no fluxo natural), enquanto PSNR e SSIM consecutivos permanecem relativamente próximos (o inpainting é determinístico e aplica a mesma máscara em cada frame, gerando preenchimentos similares entre frames adjacentes). Essa análise valida que as métricas não apenas calculam valores, mas detectam modificações visuais consistentes com o processo de inpainting~---~comportamento análogo ao que seria observado ao comparar o input e o output do VOID.
