...
Flow cons. L1       & 0.00562 & 0.00084 & 0.00440 & 0.00707 \\
PSNR consec. (dB)   & 28.632  & 1.726   & 26.270  & 31.695  \\
SSIM consec.        & 0.92535 & 0.02340 & 0.88417 & 0.95982 \\
\bottomrule
\end{tabular}
\end{table}

Os valores indicam comportamento temporal coerente: LPIPS médio $\approx 0.019$ (abaixo do limiar perceptual humano estimado em $0.05$--$0.10$), erro de fluxo médio $\approx 0.006$ (próximo de zero), PSNR médio $\approx 28.6$~dB (aceitável para vídeo de qualidade razoável~\cite{szeliski}) e SSIM médio $\approx 0.925$ (preservação estrutural consistente).

Nenhum par foi classificado como anomalia pelo detector de desvio padrão ($z > 2.0$), confirmando estabilidade temporal uniforme.
...
\midrule
Sem degradação     & 0.01894 & 0.00562 & 28.632 & 0.92535 \\
Ruído $\sigma$=0.02 & 0.02780 & 0.00841 & 24.103 & 0.88213 \\
Ruído $\sigma$=0.05 & 0.05612 & 0.01432 & 19.872 & 0.76504 \\
Ruído $\sigma$=0.10 & 0.10934 & 0.02781 & 14.221 & 0.54318 \\
JPEG q=50          & 0.02341 & 0.00634 & 26.871 & 0.91204 \\
JPEG q=20          & 0.04102 & 0.00912 & 23.415 & 0.84731 \\
Blur k=3           & 0.02187 & 0.00598 & 27.341 & 0.90812 \\
Blur k=7           & 0.03974 & 0.00843 & 24.812 & 0.85231 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item \textbf{LPIPS} cresce monotonicamente com o nível de degradação — de $0.019$ (baseline) a $0.109$ (ruído $\sigma=0.10$), um aumento de $\approx$~5.8$\times$.
    \item \textbf{Flow Consistency L1} também cresce com a degradação, refletindo inconsistência de movimento introduzida pelo ruído.
    \item \textbf{PSNR} decresce monotonicamente — de $28.6$~dB a $14.2$~dB sob ruído máximo.
    \item \textbf{SSIM} decresce de $0.925$ a $0.543$ no cenário mais severo.
\end{itemize}

O ruído gaussiano é o tipo de degradação que mais impacta todas as métricas simultaneamente, enquanto o blur afeta principalmente LPIPS e SSIM com menor impacto no PSNR — consistente com o fato de que o blur preserva energia pixelwise mas destrói estrutura de alta frequência. A compressão JPEG tem comportamento intermediário.
